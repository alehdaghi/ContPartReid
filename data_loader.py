...
class SYSUData(data.Dataset):
    def __init__(self, data_dir, transform=None, colorIndex=None, thermalIndex=None):
        ...
        train_color_image = np.load(data_dir + 'train+Val_rgb_resized_img.npy')
        self.train_color_label = np.load(data_dir + 'train+Val_rgb_resized_label.npy')

        train_thermal_image = np.load(data_dir + 'train+Val_ir_resized_img.npy')
    ...
    def __getitem__(self, index):
        ...
        img1_0, parts1_0 = self.transform_color(img1, parts1)
    ...
    def common(self, img, part):
        ...
        img = TF.pad(img, 10)
        part = TF.pad(part, 10, fill=255)
        i, j, h, w = transforms.RandomCrop.get_params(img, output_size=(288, 144))
        img = TF.crop(img, i, j, h, w)
        part = TF.crop(part, i, j, h, w)
    ...
    def transform_color1(self, img, part):
        # ChannelRandomErasing(probability=0.5)]
        # ChannelExchange(gray = 2)
        img, part = self.common(img, part)
        img, param = self.erase(img)
